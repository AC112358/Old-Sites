...
def main():
    ...
    try:
        ...
        if "attack" in form and "attacker" in form and "target" in form and "move" in form:
            print "<html><body>lets crash this thing</body></html>"
            outputs = attack(form.getvalue("attacker"), form.getvalue("target"), form.getvalue("move"),avatars)
            output = outputs[0]
            avatars=outputs[1]
            html = html.replace("<!--output-->", output+"<!--comment--><!--output-->")
            writeHTML(html)
        error = "printing"
        printHTML(html,avatars)
        print "<!--"+str(writeAvatars(avatars))+"-->"
    except:
        print getHTML()
def main2(form):
    print "Content-type: text/html"
    ...
    html = getHTML()
    if "unlock" in form and "name" in form and "element" in form:
        avatars = unlockElement(form["name"], form["element"],avatars)
    if "attack" in form and "attacker" in form and "target" in form and "move" in form:
        print "OK DOING THE THING"
        outputs = attack(form["attacker"], form["target"], form["move"],avatars)
        output = outputs[0]
        avatars=outputs[1]
...
def makeDropdown(name,avatars):
    dropdown = "<form method = get action = ChemGame3.py>"

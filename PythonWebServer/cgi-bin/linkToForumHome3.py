...
def main2(userID):
    ...
    if userID>=0:
        newHTML=newHTML.replace("<!--End Thread Names-->", addThreadButton(userID)+"\n<!--End Thread Names-->")
    #newHTML=newHTML.replace("<!--End Thread Names-->", addThreadButton(userID)+"\n<!--End Thread Names-->")
    print newHTML

def addThreadButton(userID):
    allCategories = ""
    names = os.listdir("Saved_Threads")
    for j in range(len(names)):
        names[j] = '%s'%(" ".join(names[j].split("_")))
        if j!=0 and names[j]!="staffChat":
            allCategories+="<option>%s</option>\n"%(names[j])
        elif names[j]!="staffChat":
            allCategories+="<option selected>%s</option>\n"%(names[j])
    inputControls ='''
<form method = post action = createThread.py>
<input type = text size = 40 name = threadName value = "Default Thread Name - NO underscores allowed">
